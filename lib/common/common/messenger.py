...
class Messenger:
    ...
    async def v2_pub_channel(self, namespace: NameSpaceInput, topic: Any, obj: object, **ids):
        ...
        # await self._redis.publish(channel, json.dumps({'ids': ids, 'obj': obj}))
        ret = await self._redis.publish(channel, json.dumps(obj))
        return ret
    ...
    def pub_channel(self, category: TableNames | str, sub: Category, obj: object, channel_id: int = None):
        ...
        return asyncio.create_task(self._redis.publish(ch, json.dumps(obj)))
