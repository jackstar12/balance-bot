...
from database.dbmodels.discord.guild import Guild
...
class ClientRedis:
    ...
    async def redis_set(self, keys: dict[RedisKey, Any], space: Literal['cache', 'normal']):
        ...
        mapping = {
            k.key: json.dumps(v.dict()) if isinstance(v, PydanticBaseModel) else v
    ...
    async def set_balance(self, balance: Balance):
        await self.redis_set(
            {
                RedisKey(TableNames.BALANCE): json.dumps(balance.serialize())
            },
            space='normal'
        )
